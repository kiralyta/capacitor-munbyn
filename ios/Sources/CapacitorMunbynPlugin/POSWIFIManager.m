...
-(void)POSWriteCommandWithData:(NSData *)data {
    // Ensure socket operations are performed on the main thread
    dispatch_async(dispatch_get_main_queue(), ^{
        if (_connectOK) {
            NSLog(@"----%@", data);
...
- (void)POSSendMSGWith:(NSString *)str {
    NSString *blockStr = str;

    // Ensure socket operations are performed on the main thread
    dispatch_async(dispatch_get_main_queue(), ^{
        if (_connectOK) {
            // Log the modified string for debugging
            NSLog(@"%@", blockStr);
...
-(void)POSConnectWithHost:(NSString *)hostStr port:(UInt16)port completion:(POSWIFIBlock)block {
    dispatch_async(dispatch_get_main_queue(), ^{
        // Check if _sendSocket is initialized
        if (!self.sendSocket) {
            // Initialize _sendSocket if necessary
        ...
        NSLog(@"Port number: %u", _port); // Use %u for unsigned integers like UInt16

        NSError *error = nil;
        _connectOK = [self.sendSocket connectToHost:hostStr onPort:_port withTimeout:3 error:&error];
        ...
        if (!_connectOK) {
            NSLog(@"%@", error);
            [self showAlert:@"Mumbyn printer connection failed"];
        } else {
            NSLog(@"Mumbyn printer connection success!");
        }
